import mongoose, { Connection, ConnectOptions, Model, MongooseOptions, Schema } from 'mongoose';
import * as Database from '../../../../app/config/database';
...
export const MongooseConnections = () => globalsWithMongoose.mongooseConnections
if(!globalsWithMongoose.mongooseConnections) globalsWithMongoose.mongooseConnections = {}
...
export function ConnectDatabase(type: string) {
    ...
    if (env("APP_ENV") == 'local') console.debug(`Creating database connection ${type}`)
...
const DatabaseSchemaPlugin = (schema: mongoose.Schema<any>, opts: any) => {
    ...
    if (plugins) plugins.forEach((Plugin: (arg0: mongoose.Schema<any, mongoose.Model<any, any, any, any, any, any>, {}, {}, {}, {}, mongoose.DefaultSchemaOptions, { [x: string]: unknown; }, mongoose.Document<unknown, {}, mongoose.FlatRecord<{ [x: string]: unknown; }>> & mongoose.FlatRecord<{ [x: string]: unknown; }> & Required<{ _id: unknown; }>>, arg1: any) => any) => Plugin(schema, opts));
};
