...
import { MIGRATION_PATH } from '../../../../app/config/core';
...
export class MigrationsController {
    ...
    public createMigration(name?: string) {
        ...
        const migrationPath = path.join(MIGRATION_PATH, `${Date.now()}_${name}.ts`)
    ...
    public async execMigrations() {
        ...
        if(Object.keys(toRun).length == 0){
            this.prepare_migrations(false)?.map((migration)=>{
                toRun[`mig${migration}`] = require(`${MIGRATION_PATH}/${migration}`)
            })
        }
