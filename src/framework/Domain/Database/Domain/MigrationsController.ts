...
export const globalsWithMigrations = global as typeof globalThis & {
  sensortMigrationsPath: string;
};

export const SetMigrationsPath = (path: string) => {
  globalsWithMigrations.sensortMigrationsPath = path;
};
...
export class MigrationsController {
  ...
  public createMigration(name?: string) {
    ...
    const migrationPath = path.join(
      globalsWithMigrations.sensortMigrationsPath,
      `${Date.now()}_${name}.ts`,
    );
    ...
    if (Object.keys(toRun).length == 0) {
      this.prepare_migrations(false)?.map((migration) => {
        toRun[`mig${migration}`] = require(
          `${globalsWithMigrations.sensortMigrationsPath}/${migration}`,
        );
      });
    }
  ...
  public prepare_migrations(writeFile = true) {
    ...
    const basePath = globalsWithMigrations.sensortMigrationsPath;
