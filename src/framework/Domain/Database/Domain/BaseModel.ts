import mongoose, { Document, FilterQuery, UpdateQuery, IfAny, Model, PopulatedDoc, Require_id, SchemaDefinition, SchemaDefinitionType, Types, PopulateOptions } from "mongoose";
import { Schema } from "mongoose";
import DatabaseSchemaPlugin, { ConnectDatabase, globalsWithMongoose } from "./DatabaseSchemaPlugin";
import { default as database_config } from "../../../../app/config/database";
export type DBConnectionPool = keyof typeof database_config
...
export default class BaseModel<T, ModelMethods = {}> {
    ...
    protected collection(connection?: DBConnectionPool): ModelType<T, ModelMethods> {
        if (!connection)
            connection = Object.keys(database_config)
                .map(key => ({ key, ...database_config[key] }))
    ...
    public load(): (connection?: DBConnectionPool) => ModelType<T, ModelMethods> {
        ...
        if (!globalsWithMongoose.mongooseSchemas[this.collection_name]) {
            ...
            if (nSchema.plugin) nSchema.plugin(DatabaseSchemaPlugin, {
                ...this,
                omit: [
                    ...this.ignoreDefaultFields ? ['_id', 'created', 'updated'] : [],
                    ...this.ignoredFieldsAudit || []
                ]
            })
            methods.map((method) => ([
                method.toString(),
            ...
            .forEach((k:any) => nSchema.static(k[0], (...args: any) => k[1](...args)))
    ...
    public populated(query: FilterQuery<T>, and?: DefPopulateOptions<T>) {
        ...
        if (this.populate.length > 0) return find.populate([...this.populate,...(and||[])])
        return find
    }
