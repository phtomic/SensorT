...
import {
  UUIDBitSize,
  UUIDEncoding,
} from '../interfaces/UUIDGeneratorInterface';
...
export default class Crypt {
  ...
  public static generateKey(save_env = false) {
    const key = [
      KEY_ENCODING,
      Buffer.from(
        JSON.stringify([randomBytes(32), randomBytes(16), 'aes-256-cbc']),
      ).toString(KEY_ENCODING),
    ].join(':');
    if (save_env) {
  ...
  public static generateToken() {
    return btoa(
      this.oneWayEncrypt(
        new UUIDGenerator(UUIDEncoding.BASE62, UUIDBitSize.B512).generate(),
      ),
    );
  }
  private static getAppKey() {
    const APP_KEY: any = env('APP_KEY');
    if (isEmpty(APP_KEY)) throw new Error('APP_KEY ENV not set');
    const [ENCODING, APP_KEY_JSON] = APP_KEY.split(':');

    return JSON.parse(
      atob(decodeURI(Buffer.from(APP_KEY_JSON, ENCODING).toString())),
    );
  }
  // Decrypting text
