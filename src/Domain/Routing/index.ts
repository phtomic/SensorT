...
export class Routing {
    ...
    public endpoints: Array<[string, string, Array<(req?: Request, res?: Response) => Promise<any>>] | Routing> = []
    ...
    public middleware(middlewares: AnyPluginMidleware) {
    ...
    public plugin(plugins: AnyPluginMidleware) {
    ...
    private newRoute(
        ...
        ).map((middleware) => (req: Request, res: Response) => new middleware().handle(req, res));
        execs.push(() => callback());
        ...
        this.controller.use((req: Request, res: Response, next: NextFunction) => {
            try {
                SessionStorage(async () => {
                    ...
                    setStorage('headers', req.headers);
                    next();
                });
            } catch (err) {
...
export function getHeaders() {
    return getStorage<IncomingHttpHeaders>('headers');
}
export function setSessionRequest(req: Request) {
    setStorage('request', req);
    setStorage('originalUrl', req?.originalUrl);
    setStorage('access-origin', req.headers.origin);
    setStorage('headers', req.headers);
}
export function setSessionResponse(res: Response) {
